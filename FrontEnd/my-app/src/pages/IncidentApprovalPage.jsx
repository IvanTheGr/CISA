...
import { fetchIncidentApprovalApi } from "../api/incident_approval_api";
...
function StatusBadge({ status }) {
  const text = status || "submit";
  const isApproved = String(text).toLowerCase().includes("approve");
...
function ApprovalTable({ title, subtitle, icon: Icon, tickets, type, onOpen }) {
  return (
...
function IncidentApprovalPage() {
  ...
  const loadApproval = async () => {
    try {
      setLoading(true);
      const res = await fetchIncidentApprovalApi();

      setPendingApproval(res.data?.pendingApproval || []);
      setApproved(res.data?.approved || []);
    } catch (err) {
      console.error("Failed load incident approval", err);
      setPendingApproval([]);
      setApproved([]);
    } finally {
      setLoading(false);
  ...
  const filterTickets = (tickets) => {
    ...
    return tickets.filter((ticket) => {
      return (
        ...
        String(ticket.assignedPic || "").toLowerCase().includes(keyword)
      );
    });
  };
  ...
  const openDetail = (ticketId) => {
    navigate(`/ticket/approval/${ticketId}`);
  };
      ...
      <div className="max-w-[1500px] mx-auto space-y-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">
                Approval manager untuk Incident Log staff
              </p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
          <div className="relative max-w-xl">
            ...
            <input
              ...
              placeholder="Search incident approval..."
        ...
        {loading ? (
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 text-center text-slate-500">
            Loading incident approval...
          </div>
        ) : (
          <>
            <ApprovalTable
              title="Menunggu Approve Manager"
              subtitle="Incident Log yang sudah disubmit staff dan menunggu approval"
              icon={FiClock}
...
export default IncidentApprovalPage;